...
SetStep.prototype.init=function(arg){
    ...
    if(this.opt.clickAble){
     ...
     $(window).resize(function(){
        ...
        stepsHtml.find('li').css('width','75px').css('marginRight',w_li*2-75)
...
//初始化 生成页面的进度条和按钮
SetFailStep.prototype.init=function(arg){
    var _that=this;
    extend(this.opt,arg);
    this.opt.stepCounts=this.opt.steps.length;
    this.content=$(this.opt.content);
...
//设置进度条
SetFailStep.prototype.setProgress=function(n,curIndex,stepsLen){
      var _that=this;
        //获取当前容器下所有的步骤
        var $steps = $(n).find("li");
        var $progress =$(n).find(".ystep-progress-highlight");
...
SetFailStep.prototype.checkPage=function(pageCont,curStep,steps){
    for(var i = 1; i <= steps; i++){
        if(i === curStep){
          pageCont.find('#page'+i).css("display","block");
        }else{
