...
namespace SS.Home.Core
{
    public static class Utils
    {
        ...
        public static GroupInfo GetDefaultGroupInfo(ConfigInfo configInfo)
        {
            return new GroupInfo
            {
                Id = 0,
                GroupName = "默认用户组",
                IsWriting = configInfo.IsWriting,
                WritingAdmin = configInfo.WritingAdmin
            };
        }
        ...
        public static Dictionary<string, object> ToDict(IContentInfo contentInfo)
        {
            var result = new Dictionary<string, object>();
            if (contentInfo != null)
            {
                var nvc = contentInfo.ToNameValueCollection();
    ...
    {scripts}
        ...
        public static void CopyDirectory(string sourcePath, string targetPath, bool isOverride)
        {
            ...
            foreach (var fileSystemInfo in directoryInfo.GetFileSystemInfos())
            {
                ...
                {
                    File.Copy(fileSystemInfo.FullName, destPath, isOverride);
                }
